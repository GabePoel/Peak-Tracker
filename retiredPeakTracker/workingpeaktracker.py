...
userposlist = np.array([], dtype=np.integer)
userwidthlist = np.array([], dtype=np.integer)
useramplist = np.array([], dtype=np.integer)
totalpeaklist = []
totaltemplist = []
...
def initializearray(filepath):
    ...
    fullparamlist = np.array([], dtype=np.integer)
...
def cleanBadParams(params):
    global totalMinFreq
    global totalMaxFreq
    newParams = np.array([], dtype=np.integer)
...
def trackpeaks(initparamlist, filepaths):
    ...
    for i in range(0, len(filepaths)):
        ...
        predictedPeaks = findNearest(freq, newparamlist[2::4]).astype(int)

        plt.clf()
        plt.plot(freq, R, picker=5)
        plt.plot(freq[predictedPeaks], R[predictedPeaks], "x")
        plt.draw()
        plt.pause(0.05)
        # plt.close()
...
def findpeaks(filepath):
    ...
    peaklist = np.array([], dtype=np.integer)
    for max in maxes:
